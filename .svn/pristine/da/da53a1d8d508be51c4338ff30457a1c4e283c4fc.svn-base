...
public class SubtopicServiceImpl implements ISubtopicService {
	...
	public Subtopic update(Subtopic subtopic) {
		...
		Subtopic editableSubtopic = subtopicDao.findById(Subtopic.class,
				subtopic.getId());
	...
	public void delete(Subtopic subtopic) {
		...
		List<Article> articles = articleDao.findAll(Article.class);
		if (articles.size() > 0) {
			for (Article article : articles) {
				if (article.getSubtopic().equals(subtopic)) {
