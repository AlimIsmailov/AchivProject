...
public class UserDaoImpl extends GenericDaoImpl<User> implements IUserDao {
	...
	public List<User> findByActiveStatus() {
		...
		try {
			foundresult = (List<User>) entityManager
					.createNamedQuery("User.findByActiveStatus", User.class).getResultList();
		} catch (NoResultException e) {
			return null;
		} catch (NonUniqueResultException e) {
			return null;
	...
	public List<User> findByUserRequest(UserSearchRequestForm searchRequest) {
		...
		if (searchRequest.isSearchByName()) {
			...
			//7
			mainPredicate = cb.and(cb.equal(userRoot.get("lastName"), surName), 
		...
		if (searchRequest.isAdvancedMode()) {
			...
			if (selectedRoles.size() > 1) {
				...
				for (Role role : selectedRoles) {
					if (i == 0)
						rolesPredicate = cb.isMember(role, userRoot.<Set<Role>> get("roles"));
					else {
						if (operationOnRoles.equals("AND")){
							rolesPredicate = cb.and(rolesPredicate,	cb.isMember(role,
											userRoot.<Set<Role>> get("roles")));}
						
						if (operationOnRoles.equals("OR")){
							rolesPredicate = cb.or(rolesPredicate, cb.isMember(role,
											userRoot.<Set<Role>> get("roles")));}
					}
					i++;
				}
			} else
				rolesPredicate = cb.isMember(selectedRoles.iterator().next(), userRoot.<Set<Role>> get("roles"));
			
				if (!statusOption.equals("any")) {
					statusPredicate = cb.equal(userRoot.get("active"), Integer.parseInt(statusOption));
					
					if (mainPredicate == null) 
						summary = cb.and(statusPredicate, rolesPredicate);
					 else 
						summary = cb.and(statusPredicate, mainPredicate, rolesPredicate);
