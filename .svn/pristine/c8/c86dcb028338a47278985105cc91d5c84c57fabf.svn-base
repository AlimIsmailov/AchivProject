...
public class GroupServiceImpl implements IGroupService {
	...
	public List<Group> getChosenGroups(GroupFormBean groupFormBean, String login) {
		...
		logger.info("isFuture (from GroupServiceImpl): " + isFuture);
		logger.info("isCurrent (from GroupServiceImpl): " + isCurrent);
		logger.info("isFinished (from GroupServiceImpl): " + isFinished);
		List<Group> groupList = new ArrayList<Group>();

		if (isFuture) {
			logger.info("if isFuture");
			if (isAdmin == true) {
				groupList.addAll(findFutureGroups());
				logger.info("GroupList (If isAdmin) " + groupList.toString());
			} else {
		...
		logger.info("FINAL GROUP LIST(FROM GROUPSERVICE: " + groupList);
	...
	@Override
	public void addGroup(GroupFormBean groupFormBean) throws ElementExistsException, InvalidDateException {
	...
	@Override
	public void editGroup(GroupFormBean groupFormBean)
		...
		Set<User> managers = userService.stringToManagers(groupFormBean
				.getUsers());
		Direction direction = directionService.findByName(groupFormBean.getGroup().getDirection().getName());
	...
	@Override
	public void applyToGroup(Long groupId, String login) throws ElementExistsException {
