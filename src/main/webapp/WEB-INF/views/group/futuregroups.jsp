...
<script type="text/javascript">
	$(document).ready(function() {
		$('.apply').click(function() {
			document.getElementById("apply").innerHTML="<p>Applied</p>";
		})
	})
</script>
			<c:if test="${success eq true}">
				<div class="alert alert-success"><spring:message code="label.appliedSuccess" /></div>
			</c:if>
			<c:if test="${fail eq true}">
				<div class="alert alert-danger"><spring:message code="label.appliedFail" /></div>
			</c:if>
					...
					<c:forEach items="${futureGroups}" var="group">
						<tr>
							...
							<security:authorize
								...
								<td><a href="group/apply/${group.id}"
									class="btn btn-default" class="apply"> <spring:message
